...
userSecure.post('/secure-users', async (req, res) => {
  ...
  const user = await User.create({ tenantId, name, email, passwordHash, permissions: permissions || ['users.read','users.write','products.read','products.write','stock.write','reports.read'] });
