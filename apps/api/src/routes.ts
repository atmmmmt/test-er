...
routes.get('/activities', requirePermission('reports.read'), aw(async (req: any, res: any) => res.json({ data: await Activity.find({ tenantId: tid(req) }).limit(100).sort({ createdAt: -1 }) })));
...
routes.post('/stock/receive', requirePermission('stock.write'), aw(async (req: any, res: any) => { const data = await receivePurchase(req.body); await logActivity({ tenantId: String(tid(req)), userId: req.user?.userId, action: 'stock.receive', entity: 'stock', message: 'Stock received', meta: req.body }); res.status(201).json({ data }); }));
routes.post('/stock/issue', requirePermission('stock.write'), aw(async (req: any, res: any) => { const data = await issueSale(req.body); await logActivity({ tenantId: String(tid(req)), userId: req.user?.userId, action: 'stock.issue', entity: 'stock', message: 'Stock issued', meta: req.body }); res.status(201).json({ data }); }));
routes.post('/stock/move', requirePermission('stock.write'), aw(async (req: any, res: any) => { const data = await moveStock(req.body); await logActivity({ tenantId: String(tid(req)), userId: req.user?.userId, action: 'stock.move', entity: 'stock', message: 'Stock moved', meta: req.body }); res.status(201).json({ data }); }));
...
routes.get('/reports/overview', requirePermission('reports.read'), aw(async (req: any, res: any) => {
  ...
  const [tenants, plans, subscriptions, users, products, warehouses, suppliers, customers, purchases, sales, movements, lowStock, activities] = await Promise.all([Tenant.countDocuments(), Plan.countDocuments(), Subscription.countDocuments(filter), User.countDocuments(filter), Product.countDocuments(filter), Storage.countDocuments(filter), Party.countDocuments({ ...filter, type: 'supplier' }), Party.countDocuments({ ...filter, type: 'customer' }), PurchaseOrder.countDocuments(filter), Sale.countDocuments(filter), Movement.countDocuments(filter), lowStockAlerts(String(tid(req))), Activity.countDocuments(filter)]);
  res.json({ data: { tenants, plans, subscriptions, users, products, warehouses, suppliers, customers, purchases, sales, movements, lowStock: lowStock.length, activities } });
}));
